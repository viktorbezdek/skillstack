...
Load the `skill-creator` skill. It knows the current conventions for well-formed skills and will catch mistakes that would take you weeks to diagnose on your own.
...
- For managing the full plugin bundle (not just one skill), use the skillstack repo's `skill-creator` plugin directly
